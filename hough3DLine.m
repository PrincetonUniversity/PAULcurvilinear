...
if max(H(:))~=0 && max(H(:))~=1 % added200405: scattered segments are noise, skip while loop
    while max(Htemp(:))>= H_Threshold
        %     [max(Htemp(:)) H_Threshold] % display these two for debugging
        % select peak; if multiple, select any max point
        [az_ind_temp,el_ind_temp,xp_ind_temp,yp_ind_temp]=ind2sub(size(H),find(Htemp==max(Htemp(:)),1));
        % caution: the indices may be at the edge, in that case the neighbor
        % one needs to turn around to the end (for angles); avoid negative rho
        ...
        if length(az_ind) > numpeaks 
            break; %added 200606: select "numpeaks" max H
        end
    end
end
% no non-max suppression
% [az_ind,el_ind,xp_ind,yp_ind]=ind2sub(size(H),find(H>=Threshold_ratio*max(H(:))));
...
for l = 1:length(az_ind)
    ...
    % find the most distant two points
