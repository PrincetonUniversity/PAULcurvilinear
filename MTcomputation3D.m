...
%% Feature enhancement requires a grayscale input, or color channel at dim #4
% if size(image3D,4) == 3
%     image3D = rgb2gray3D(image3D);
% end
% clock
...
disp('Image enhanced');
%save('enhanced','FeatureEnhancedIM');
% clock
...
% separate out situations with different z gridding (assume such problems
% wouldn't appear with x or y direction)
...
% restore to symmetric division (linear wrt division number)
...
% save('all_segs','AllShiftsPt1x','AllShiftsPt1y','AllShiftsPt1z',...
%     'AllShiftsPt1z','AllShiftsPt2x','AllShiftsPt2y','AllShiftsPt2z');
%save('all_segs','AllPt1x','AllPt1y','AllPt1z',...
%    'AllPt1z','AllPt2x','AllPt2y','AllPt2z');
% clock

% calculate all segment lengths
% D = pdist2([AllShiftsPt1x AllShiftsPt1y AllShiftsPt1z],...
%     [AllShiftsPt2x AllShiftsPt2y AllShiftsPt2z]);
% AllSegLengths = diag(D);
%======plot the results (all line segments from all shifts together)=======
% figure;
% imshow(FeatureEnhancedProj); hold on
% plot3([AllShiftsPt1y';AllShiftsPt2y'],...
%     [AllShiftsPt1x';AllShiftsPt2x'],...
%     [AllShiftsPt1z';AllShiftsPt2z'],'Color','k','LineWidth',1);
% axis image
% box on
% xlabel('x');ylabel('y');zlabel('z');

% plot in real space
% plot3(([AllShiftsPt1y';AllShiftsPt2y']-0.5).*0.076,...
%     ([AllShiftsPt1x';AllShiftsPt2x']-0.5).*0.076,...
%     ([AllShiftsPt1z';AllShiftsPt2z']-0.5).*0.23,'Color','k','LineWidth',1);
%==========================================================================
% curr
%% Grouping
% Remove Edge Noises
...
colinearInd = (maxnorm==0); % Colinear => the final k value should be exactly 0
% 0610: if far away, lenRatio is the only thing that can get rid of them
...
% Plot groups
% colors = hsv(length(uniqueGps));
% for i=1:max(groupInd)
%     CurrGroupInd=find(groupInd==i);
%     plot3([Pt1(CurrGroupInd,2)';Pt2(CurrGroupInd,2)'],...
%         [Pt1(CurrGroupInd,1)';Pt2(CurrGroupInd,1)'],...
%         [Pt1(CurrGroupInd,3)';Pt2(CurrGroupInd,3)'],'Color',colors(i,:));
%     hold on
% end
% box on
% axis image
disp('Line segments grouped');
%save('grouping','Pt1','Pt2','groupInd');
% clock
